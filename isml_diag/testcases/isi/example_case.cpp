...
TestStatus ISIModule::example(TestInfo& ti)
{
    ...
    const auto timeout_us = static_cast<uint32_t>(common::args::get_u64(ti.args, "timeout_us"));
    const auto native = phal_component_isi_linkup(static_cast<phal_ctx_t*>(ctx.phal), timeout_us);
    if (native != PHAL_STATUS_OK)
        ti.logger->error("phal_component_isi_linkup failed: status=" + std::to_string(static_cast<int>(native)));
    status |= from_phal(static_cast<int>(native));
